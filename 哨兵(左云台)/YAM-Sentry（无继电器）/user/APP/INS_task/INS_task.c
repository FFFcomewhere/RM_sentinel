...
static uint8_t Target_temperate;
...
//将读取到的ist8310原始数据处理成国际单位的数据
        ...
        //请在这里添加例如温度控制代码
        get_control_temperate();
        IMU_temp_Control(Target_temperate);
...
float get_control_temperate(void)
{
	Target_temperate = (int8_t)(cali_get_mcu_temperature()) + 10;
	        if (Target_temperate > (int8_t)(GYRO_CONST_MAX_TEMP))
        {
            Target_temperate = (int8_t)(GYRO_CONST_MAX_TEMP);
        }
}
